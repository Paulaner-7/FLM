// FLM — Hub Competizioni (PRD 7.1): tutte le competizioni della stagione.
// Per competizione: classifica (campionato), tabella league phase (coppe UEFA),
// tabellone a eliminazione (coppe), report sorteggio, classifiche speciali.
...
import type { Competizione, Giocatore, Id, Partita, PrestazionePartita, Squadra } from '../types/entities';
...
export default function Competizioni({ carrieraId, onBack }: CompetizioniProps): ReactElement {
  ...
  useEffect(() => {
    let attivo = true;
    void (async () => {
      const [competizioni, squadre, giocatori] = await Promise.all([
  ...
  return (
    <main className="page-shell">
      <header className="topbar">
        <button className="brand-button" type="button" onClick={onBack}>FLM <span>/ Competizioni</span></button>
        <div className="topbar-actions">
          <span className="topbar-note">Stagione {dati.competizioni[0]?.stagione ?? ''}</span>
        </div>
      </header>
      ...
      <section className="content-wrap">
        {competizione === null ? (
          <>
            <p className="eyebrow">Tutte le competizioni</p>
            <h1>Competizioni</h1>
            <p className="intro">Classifiche, league phase, tabelloni e classifiche speciali di ogni competizione della stagione.</p>
            <div className="comp-list">
              {dati.competizioni.map((c) => (
                <button key={c.id} type="button" className="comp-card" onClick={() => setSelezionata(c.id)}>
                  <strong>{c.nome}</strong>
                  <span>{c.fase === 'conclusa' ? 'Conclusa' : `Fase: ${nomeFaseLeggibile(c.fase as never)}`}</span>
                  <span>{c.squadre.length} squadre</span>
                </button>
              ))}
            </div>
          </>
...
function DettaglioCompetizione({
  ...
  return (
    <>
      <button className="button button-quiet" type="button" onClick={onIndietro}>← Tutte le competizioni</button>
      <p className="eyebrow">{competizione.tipo === 'campionato' ? 'Campionato' : competizione.nome}</p>
      <h1>{competizione.nome}</h1>

      <div className="mode-row" role="group" aria-label="Viste competizione">
        <button type="button" className={`mode-chip ${tab === 'classifica' ? 'mode-chip-active' : ''}`} onClick={() => setTab('classifica')}>Classifica</button>
        {partiteKo.length > 0 && (
          <button type="button" className={`mode-chip ${tab === 'tabellone' ? 'mode-chip-active' : ''}`} onClick={() => setTab('tabellone')}>Tabellone</button>
        )}
        {competizione.fasce && competizione.fasce.length > 0 && (
          <button type="button" className={`mode-chip ${tab === 'sorteggio' ? 'mode-chip-active' : ''}`} onClick={() => setTab('sorteggio')}>Sorteggio</button>
        )}
        <button type="button" className={`mode-chip ${tab === 'speciali' ? 'mode-chip-active' : ''}`} onClick={() => setTab('speciali')}>Classifiche speciali</button>
      </div>
          ...
          <table className="standings-table">
            ...
            <tbody>
              {classificaTipo().map((r) => (
                <tr key={r.squadraId} className={competizione.formato === 'league_phase' && (r as { qualificazione?: string }).qualificazione === 'eliminata' ? 'standings-out' : ''}>
                  <td>{r.posizione}</td>
          ...
          {competizione.formato === 'league_phase' && (
            <p className="referto-hint">
              ...
              {competizione.fase === 'conclusa' && competizione.vincitoreId && (
                <> Vincitrice: <strong>{nome(competizione.vincitoreId)}</strong>.</>
              )}
            </p>
          )}
        </div>
      ...
      {tab === 'tabellone' && (
        <div className="bracket-wrap">
          {tabelloneFasi.map((fase) => {
            ...
            return (
              <section key={fase} className="bracket-round">
                <p className="eyebrow">{nomeFaseLeggibile(fase as never)}</p>
                {[...perSfida.entries()].sort((a, b) => a[0] - b[0]).map(([n, sfida]) => {
                  ...
                  return (
                    <div key={n} className={`bracket-tie ${andata.giocata ? '' : 'bracket-tbd'}`}>
                      <strong>{nome(andata.casa)}</strong>
                      <span className="bracket-score">
                        {andata.giocata ? `${totale1}–${totale2}` : '–'}
                        {rigori && ` (${rigori.casa}-${rigori.trasferta} rig.)`}
                      </span>
                      <strong>{nome(andata.trasferta)}</strong>
                    </div>
                  );
                })}
              </section>
      ...
      {tab === 'sorteggio' && competizione.fasce && (
        <div className="draw-wrap">
          {competizione.fasce.map((fascia, i) => (
            <section key={i} className="draw-pot">
              <p className="eyebrow">Fascia {i + 1}</p>
              <ul>
                {fascia.map((id) => (
                  <li key={id}>{nome(id)} <small>coeff. {dati.squadre.get(id)?.coefficiente ?? '—'}</small></li>
                ))}
              </ul>
            </section>
          ))}
      ...
      {tab === 'speciali' && (
        <div className="speciali-wrap">
          <h2 className="risultati-h2">Classifiche speciali</h2>
          <div className="speciali-grid">
...
function ClassificaSpeciale({
  ...
  return (
    <section className="speciali-card">
      <p className="eyebrow">{titolo}</p>
      {righe.length === 0 ? (
        <p className="giornale-vuoto">Nessun dato.</p>
      ) : (
        <ol>
          {righe.map((r) => (
            <li key={r.giocatoreId}>
              <span className="speciali-nome"><strong>{nome(r.giocatoreId)}</strong> <small>{squadra(r.giocatoreId)}</small></span>
              <span className="speciali-valore">{valore(r)}{titolo === 'Voto medio' ? '' : r.presenze > 0 ? ` (${r.presenze} pg)` : ''}</span>
            </li>
          ))}
        </ol>
      )}

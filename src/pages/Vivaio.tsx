...
export default function Vivaio({ carrieraId, onBack }: VivaioProps): ReactElement {
  ...
  // D8 — accento dinamico (prima del return)
  const primario = dati?.squadre.get(dati!.carriera.squadraId)?.colori?.primario;
  const secondario = dati?.squadre.get(dati!.carriera.squadraId)?.colori?.secondario;
  useEffect(() => {
  ...
  const squadra = squadre.get(squadraId);
  ...
  return (
    <main className="page-shell">
      <HubTopbar
        sezione="Vivaio"
        contesto={`${carriera.campionato} · ${carriera.stagione}`}
        onBrand={onBack}
        onStorico={() => {}}
        onEsporta={() => {}}
        onHome={onBack}
